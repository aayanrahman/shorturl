...
class Form extends React.Component {
    ...
    onSubmit = async(event) => {
        ...
        var isFromValid = await this.ValidateInput();
        if(!isFromValid){
            return;
        }

        //Users prefared alias should be used 
        var generatedKey = nanoid(5);
        var generatedURL = "URLshort.com/" + generatedKey;

        if(this.state.preferedAlias !== ''){
            generatedKey = this.state.preferedAlias; 
            generatedURL = "URLshort.com/" + this.state.preferedAlias;
        }
    ...
    // Save the content of the form user types 
    handleChange = (e) => {
